...
from src.opf_utils import NodeCostFunction, TOL, assert_approx
...
class Zone:
    ...
    def set_price_model(self, price_models: dict[str, list[float]]):
        ...
        for sector in self.sectors:
            if sector.name not in price_models.keys():
                ...
                sector_price_model = (0, 0)  # Default value
            else:
    ...
    def compute_demand(self, net_imports: pd.Series):
        ...
        if len(self._power_demand) == 0:
            ...
            demand_sector.set_price_model((1000, 1000))  # FIXME : what should be the demand price ?
    ...
    def update_cost_function(self, timestep: pd.Timestamp):
        ...
        # threshold_prices_list = sorted(set(filter(lambda p: p >= 0, threshold_prices_list + [0])))
        print(threshold_prices_list)
        # FIXME: There is an issue to fix with negative price threshold (which are expected to be generated by the
        #  price model algorithm.
        #  With them, the power/cost function is not always increasing anymore and it can even be negative. Which of
        #  course is not realistic.
        #  When ignoring them (filter out) there is an issue with the initialisation of last_price (used to be None,
        #  and modified in the first iteration of the loop).
        #  idea: consider them as constrained production ??

        #
        # To compute to node cost function, we build in the first place its price/power curve by considering the
        ...
        for sector_list in (real_load_names, storage_gen_names, storage_load_names, real_gen_names):
            for sector_name in sector_list:
                if sector_name not in sectors_with_power:
                    sector = next(iter(sector for sector in self.sectors if sector.name == sector_name))
                    offer_range = power_range_per_sector[sector_name]
