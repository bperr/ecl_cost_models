...
def read_interconnection_capacities_data(file_path_data: Path, file_path_map: Path) -> pd.DataFrame:
    ...
    # Load data
    df_interco_capacity_raw = pd.read_excel(file_path_data)
    ...
    # Creating the reversed DataFrame and concatenate both original and reversed data : to have for one interconnection,
    # two lines in the dataframe with each country once in country_from and once in country_to.
